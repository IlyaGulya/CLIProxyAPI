...
func (e *CodexWebsocketsExecutor) trackCodexConnection(conn *websocket.Conn) {
	if e == nil || conn == nil {
		return
	}
	if _, loaded := e.connections.LoadOrStore(conn, &sync.Once{}); !loaded {
		observability.RecordOpenConnection(1)
	}
}

func (e *CodexWebsocketsExecutor) closeCodexConnection(conn *websocket.Conn) error {
	if conn == nil {
		return nil
	}
	errClose := conn.Close()
	if value, ok := e.connections.LoadAndDelete(conn); ok {
		value.(*sync.Once).Do(func() { observability.RecordOpenConnection(-1) })
	}
	return errClose
}
...
func (s *codexWebsocketSession) activateReader(ch chan codexWebsocketRead) error {
	if s == nil {
		return fmt.Errorf("codex websocket session is nil")
	}
	if ch == nil {
		return fmt.Errorf("codex websocket reader channel is nil")
	}
	if err := s.transitionLifecycle(codexEventRequestStarted); err != nil {
		return err
	}
	s.activeMu.Lock()
...
func (s *codexWebsocketSession) deactivateReader(ch chan codexWebsocketRead) {
	if s == nil {
		return
	}
	s.activeMu.Lock()
...
func (s *codexWebsocketSession) deactivateCurrentReader() {
	if s == nil {
		return
	}
	s.activeMu.Lock()
...
func (s *codexWebsocketSession) applyLifecycle(event codexSessionEvent) {
	if err := s.transitionLifecycle(event); err != nil {
		log.WithError(err).WithFields(log.Fields{"session_id": s.sessionID, "event": event.String()}).Error("codex websockets: lifecycle event rejected")
	}
}

func (s *codexWebsocketSession) transitionLifecycle(event codexSessionEvent) error {
	if s == nil {
		return fmt.Errorf("codex websocket session is nil")
	}
...
func (e *CodexWebsocketsExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	prepared, errPrepare := e.prepareWebsocketRequest(ctx, auth, req, opts, false)
	if errPrepare != nil {
	...
	if sess != nil {
		...
		if errActivate := sess.activateReader(readCh); errActivate != nil {
			sess.reqMu.Unlock()
			return resp, fmt.Errorf("activate websocket reader: %w", errActivate)
		}
		defer sess.deactivateReader(readCh)
	}
...
func (e *CodexWebsocketsExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	prepared, errPrepare := e.prepareWebsocketRequest(ctx, auth, req, opts, true)
	if errPrepare != nil {
	...
	attemptConnection, errDial := connectCodexStreamAttempt(attemptMachine, func() (codexConnectionLease, error) {
		if sessionOverflow {
			lease, err := e.ensureOverflowConnObserved(ctx, auth, authID, wsURL, wsHeaders, executionSessionID)
	...
	if errSend != nil {
		...
		if sess != nil {
			...
			recordAPIWebsocketHandshake(ctx, e.cfg, respHSRetry)
			reporter.StartResponseTTFT()
			if errSendRetry := sendCodexAttemptRequest(attemptMachine, func() error {
				return writeCodexWebsocketMessage(sess, connRetry, wsReqBodyRetry)
			}); errSendRetry != nil {
				helps.RecordAPIWebsocketError(ctx, e.cfg, "send_retry", errSendRetry)
				e.invalidateUpstreamConn(sess, connRetry, "send_error", errSendRetry)
				sess.deactivateReader(readCh)
	...
	go func() {
		...
		delivery, errDelivery := newCodexStreamDelivery(transactionalPolicy, deliver, codexStreamDeliveryHooks{
			Committed: func(drain codexTransactionalDrain) {
				helps.RecordAPIWebsocketEvent(ctx, e.cfg, "transactional_stream_committed", observability.WebsocketAttributes{
					...
					Reason: string(drain.Boundary), TransactionalPolicy: observability.TransactionalPolicy(transactionalPolicy.name()), CommitBoundary: observability.StreamCommitBoundary(drain.Boundary),
				}, nil)
			},
			Discarded: func(discardedBytes int, discardErr error) {
				helps.RecordAPIWebsocketEvent(ctx, e.cfg, "transactional_stream_discarded", observability.WebsocketAttributes{
					SessionID: executionSessionID, Bytes: observability.Some(int64(discardedBytes)), Reason: codexTransactionDiscardReason(discardErr),
					TransactionalPolicy: observability.TransactionalPolicy(transactionalPolicy.name()),
				}, nil)
			},
			Overflowed: func(overflowBytes int) {
				helps.RecordAPIWebsocketEvent(ctx, e.cfg, "transactional_stream_overflow", observability.WebsocketAttributes{
					...
					TransactionalPolicy: observability.TransactionalPolicy(transactionalPolicy.name()), CommitBoundary: observability.StreamCommitBoundaryBufferLimit,
				}, nil)
			},
		})
		if errDelivery != nil {
			...
			if errRead != nil {
				...
				if decision.Action == codexRetryReconnect {
					...
					if errDialRetry == nil && connRetry != nil {
						...
						errSendRetry := sendCodexAttemptRequest(attemptMachine, func() error {
							return writeCodexWebsocketMessage(sess, connRetry, wsReqBodyRetry)
						})
						if errSendRetry == nil {
							conn = connRetry
						...
						} else if errClose := e.closeCodexConnection(connRetry); errClose != nil {
...
func (e *CodexWebsocketsExecutor) getOrCreateSession(sessionID string) *codexWebsocketSession {
	...
	if managedClaudeSession && maxSessions > 0 && managedSessionCount >= maxSessions {
		...
		if oldest == nil {
			store.mu.Unlock()
			for i := range toClose {
				e.closeExecutionSession(toClose[i], "session_expired")
			}
			return nil
		}
	...
	store.mu.Unlock()
	for i := range toClose {
		e.closeExecutionSession(toClose[i], "session_evicted")
	}
	return sess
}
...
func (e *CodexWebsocketsExecutor) ensureUpstreamConn(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (*websocket.Conn, *http.Response, error) {
	lease, err := e.ensureUpstreamConnObserved(ctx, auth, sess, authID, wsURL, headers)
	return lease.conn, lease.response, err
}

func (e *CodexWebsocketsExecutor) ensureOverflowConnObserved(ctx context.Context, auth *cliproxyauth.Auth, authID string, wsURL string, headers http.Header, sessionID string) (codexConnectionLease, error) {
	if pooledConn := e.takeSpeculativePreconnect(ctx, auth, authID, wsURL, headers, sessionID); pooledConn != nil {
		return codexConnectionLease{conn: pooledConn, source: codexWebsocketConnectionSpeculative}, nil
	}
	conn, resp, errDial := e.dialCodexWebsocket(ctx, auth, wsURL, headers)
	return codexConnectionLease{conn: conn, response: resp, source: codexWebsocketConnectionCold}, errDial
}

func (e *CodexWebsocketsExecutor) ensureUpstreamConnObserved(ctx context.Context, auth *cliproxyauth.Auth, sess *codexWebsocketSession, authID string, wsURL string, headers http.Header) (codexConnectionLease, error) {
	if sess == nil {
		conn, resp, errDial := e.dialCodexWebsocket(ctx, auth, wsURL, headers)
		return codexConnectionLease{conn: conn, response: resp, source: codexWebsocketConnectionCold}, errDial
	}
	...
	if conn != nil {
		...
		return codexConnectionLease{conn: conn, source: codexWebsocketConnectionSessionReuse}, nil
	}
	...
	if errDial != nil {
		...
		return codexConnectionLease{response: resp, source: codexWebsocketConnectionCold}, errDial
	}
	...
	if sess.conn != nil {
		...
		return codexConnectionLease{conn: previous, source: codexWebsocketConnectionSessionReuse}, nil
	}
	sess.conn = conn
	sess.wsURL = wsURL
	sess.authID = authID
	...
	logCodexWebsocketConnected(sess.sessionID, authID, wsURL)
	sess.applyLifecycle(codexEventConnected)
	return codexConnectionLease{conn: conn, response: resp, source: codexWebsocketConnectionCold}, nil
}
...
func (e *CodexWebsocketsExecutor) closeExecutionSession(sess *codexWebsocketSession, reason string) {
	e.closeCodexWebsocketSession(sess, reason)
}

func (e *CodexWebsocketsExecutor) closeCodexWebsocketSession(sess *codexWebsocketSession, reason string) {
	if sess == nil {
		return
	}
...
func (e *CodexWebsocketsExecutor) CloseAuthExecutionSessions(authID string, reason string) {
	...
	for i := range toClose {
		e.closeExecutionSession(toClose[i], reason)
	}
}
