...
func TestHandlerStreamInterceptorKeepsReturnedHeadersStableAfterFirstPayload(t *testing.T) {
	...
	if upstreamHeaders.Get("X-Chunk") != "" || upstreamHeaders.Get("X-Stage") != "init" {
		t.Fatalf("upstream headers after first chunk = %#v, want immutable header-init snapshot", upstreamHeaders)
	}
	...
	if upstreamHeaders.Get("X-Chunk") != "" || upstreamHeaders.Get("X-Stage") != "init" {
		t.Fatalf("upstream header-init snapshot changed after payloads: %#v", upstreamHeaders)
	}
}
